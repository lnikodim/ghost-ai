Clerk is already installed and connected. Wire it into the NextJS app: provider, auth pages, redirects,
...
Wrap the root layout with `ClerkProvider` using the Clerk's `dark` theme.

Create sign-in and sign-up pages using Clerk components.

Use `proxy.ts` at the project root, not `middleware.ts`.

Define public routes using the existing sign-in and sign-up env variables. Protect everything else by default.
...
Use existing Clerk env variables. Do not rename or invent new ones.
